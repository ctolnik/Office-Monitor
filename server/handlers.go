...
func getDailyReportHandler(c *gin.Context) {
        ...
        zapctx.Info(ctx, "🔵 getDailyReportHandler called",
                zap.String("username", username),
                zap.String("date_param", dateStr),
        ...
        zapctx.Info(ctx, "🔵 Parsed date",
                zap.String("parsed_date", date.Format("2006-01-02 15:04:05 MST")),
                zap.String("timezone", appLocation.String()))
        ...
        zapctx.Info(ctx, "🔵 getDailyReportHandler - report retrieved",
                zap.String("username", report.Username),
                zap.String("date", report.Date),
                zap.Int("activity_events", len(report.ActivityEvents)),
                zap.Int("applications", len(report.Applications)),
                zap.Int("screenshots", len(report.Screenshots)),
                zap.Int("usb_events", len(report.USBEvents)),
                zap.Int("file_events", len(report.FileEvents)),
                zap.Int("keyboard_periods", len(report.KeyboardPeriods)),
                zap.Int("dlp_alerts", len(report.DLPAlerts)))

        // Log first few items for debugging
        if len(report.ActivityEvents) > 0 {
                zapctx.Info(ctx, "🔵 First activity event",
                        zap.String("process", report.ActivityEvents[0].ProcessName),
                        zap.String("window", report.ActivityEvents[0].WindowTitle),
                        zap.Uint32("duration", report.ActivityEvents[0].Duration))
        }
        if len(report.Applications) > 0 {
                zapctx.Info(ctx, "🔵 First application",
                        zap.String("process", report.Applications[0].ProcessName),
                        zap.Uint64("duration", report.Applications[0].Duration),
                        zap.String("category", report.Applications[0].Category))
        }

        c.JSON(http.StatusOK, report)
        zapctx.Info(ctx, "🔵 getDailyReportHandler - JSON response sent")
}
...
// getActivityByUsernameHandler returns activity events for a specific user
func getActivityByUsernameHandler(c *gin.Context) {
        ctx := c.Request.Context()
        username := c.Param("username")

        fromStr := c.DefaultQuery("from", time.Now().AddDate(0, 0, -1).Format(time.RFC3339))
        ...
        events, err := db.GetActivityEventsByUsername(ctx, username, from, to)
        if err != nil {
                zapctx.Error(ctx, "Failed to get activity", zap.Error(err), zap.String("username", username))
...
// getStatsHandler returns application usage statistics for a specific user
func getStatsHandler(c *gin.Context) {
        ...
        apps, err := db.GetApplicationUsage(ctx, username, from, to)
        if err != nil {
                zapctx.Error(ctx, "Failed to get stats", zap.Error(err), zap.String("username", username))
                c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get statistics"})
                return
        }
        ...
        for _, app := range apps {
                stats[app.ProcessName] += app.Duration
        }
