...
func (db *Database) GetDailyReport(ctx context.Context, username string, date time.Time) (*DailyReport, error) {
        ...
        if err != nil {
                zapctx.Warn(ctx, "Failed to get activity segments", zap.Error(err))
                activitySegments = make([]ActivitySegment, 0)
        }

        // Convert segments to activity events for timeline compatibility
        activityEvents := make([]ActivityEvent, 0)
        for _, seg := range activitySegments {
                activityEvents = append(activityEvents, ActivityEvent{
                        Timestamp:    seg.TimestampStart,
                        ComputerName: seg.ComputerName,
        ...
        // Get applications from segments
        apps, err := db.GetApplicationUsageFromSegments(ctx, username, startOfDay, endOfDay)
        if err != nil {
                zapctx.Warn(ctx, "Failed to get applications from segments", zap.Error(err))
                apps = make([]ApplicationUsage, 0)
        }
        report.Applications = apps

        // Get application timeline (when each app was used)
        appTimeline, err := db.GetApplicationTimeline(ctx, username, startOfDay, endOfDay)
        if err != nil {
                zapctx.Warn(ctx, "Failed to get application timeline", zap.Error(err))
                appTimeline = make([]ApplicationTimeline, 0)
        }
