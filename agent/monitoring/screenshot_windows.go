...
func (m *ScreenshotMonitor) captureScreenshot() (*ScreenshotData, error) {
        // Check if we have access to the desktop (Session 0 services don't)
        if !m.hasDesktopAccess() {
                return nil, nil // Skip silently when no desktop access
        }
...
// hasDesktopAccess checks if we can access the user's desktop
// Services in Session 0 cannot access interactive desktop
func (m *ScreenshotMonitor) hasDesktopAccess() bool {
        // Try to get screen metrics - if it fails or returns 0, no desktop access
        width, _, _ := procGetSystemMetrics.Call(SM_CXSCREEN)
        height, _, _ := procGetSystemMetrics.Call(SM_CYSCREEN)
        ...
        // Try to get a DC - this will fail in Session 0
        hDC, _, _ := procGetDC.Call(0)
        if hDC == 0 {
                return false
        }
