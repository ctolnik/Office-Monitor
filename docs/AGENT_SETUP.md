...
- **НЕ отправляет никаких данных** — ждёт входа пользователя
- В логах: `Служба запущена успешно, конфиг прочитан, ожидание входа пользователя...`
...
- В логах: `Active user detected: USERNAME`, `Service is fully operational`

### Логируемые события сессий:
- Вход/выход пользователя (logon/logoff)
- Блокировка/разблокировка сессии (lock/unlock)
- Подключение/отключение консоли
- Подключение/отключение удалённого доступа (RDP)
