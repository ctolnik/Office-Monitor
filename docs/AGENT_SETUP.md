...
Служба не отправляет данные пока пользователь не войдёт в систему:
...
- Проверяет наличие активного пользователя каждые 10 секунд
- В логах: `No active user, waiting for logon (polling every 10s)...`

### При входе пользователя:
- Служба определяет имя пользователя через WTS API
- Запускаются все компоненты мониторинга
- В логах: `User logon detected: USERNAME`, `Service is fully operational`
